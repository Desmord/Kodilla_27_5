...
describe('Concerts by day', () => {
    ...
    it('Search by day wrong day', async () => {
        const res = await request(server).get('/api/concerts/day/5');
        expect(res.body.message).to.be.equal(`Not found`)
    });

    it('Search by day wrong data type', async () => {
        const res = await request(server).get('/api/concerts/day/aaa');
        expect(res.body.message).to.be.equal(`Not found`)
    });
